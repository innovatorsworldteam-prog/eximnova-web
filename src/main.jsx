...
function App() {
  ...
  const loadData = useCallback(async () => {
    ...
    const results = await Promise.all(Object.entries(routes).map(async ([key, path]) => { try { const r = await api(path); return [key, Array.isArray(r.data) ? r.data : []]; } catch { return [key, []]; } }));
    setData(current => ({ ...current, ...Object.fromEntries(results) })); setLoading(false);
  }, [token]);
  ...
  const createBusiness = async () => { const legal = prompt('Legal business name'); if (!legal) return; try { await api('/api/businesses',{method:'POST',body:JSON.stringify({legal_name:legal,brand_name:legal,entity_type:'Proprietorship',established_year:new Date().getFullYear(),description:'Created from EximNova dashboard'})}); await loadData(); setSection('Businesses'); } catch(e){setError(e.message)} };
